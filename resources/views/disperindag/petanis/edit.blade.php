...
<div class="card p-8 md:p-10 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
    <form action="{{ route('disperindag.petanis.update', $item->id) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="number" name="luas_lahan_m2" value="{{ $item->luas_lahan_m2 }}" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all">
            @error('luas_lahan_m2')<p class="mt-1.5 text-xs" >{{ $message }}</p>@enderror
        </div>
        <div>
            <label class="label-field">Jenis Komoditas</label>
            <input type="text" name="jenis_komoditas" value="{{ $item->jenis_komoditas }}" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all">
