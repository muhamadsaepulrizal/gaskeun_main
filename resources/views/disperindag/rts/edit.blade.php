...
<div class="card p-8 md:p-10 bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100">
    <form action="{{ route('disperindag.rts.update', $item->id) }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <input type="text" name="kriteria_bantuan" value="{{ old('kriteria_bantuan', $item->kriteria_bantuan) }}" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all">
        ...
        <div>
            ...
            <div class="relative mt-1 group">
                ...
                <select name="status_penerima" required class="select-field pl-11 pr-10">
                    <option value="Layak"  {{ $item->status_penerima == 'Layak' ? 'selected' : '' }}>Layak</option>
                    <option value="Tidak Layak"  {{ $item->status_penerima == 'Tidak Layak' ? 'selected' : '' }}>Tidak Layak</option>
                    <option value="Menerima"  {{ $item->status_penerima == 'Menerima' ? 'selected' : '' }}>Menerima</option>
                </select>
