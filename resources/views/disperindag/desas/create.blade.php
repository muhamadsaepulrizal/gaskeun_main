...
<div class="flex justify-end mb-6">
    <a href="{{ route('disperindag.desas.index') }}" class="inline-flex justify-center items-center rounded-xl bg-white px-6 py-3 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-200 hover:bg-slate-50 hover:text-slate-900 transition-all duration-200 shrink-0">
    ...
    <form action="{{ route('disperindag.desas.store') }}" method="POST" class="space-y-5">
        ...
        <div>
            ...
            <div class="relative mt-1 group">
                ...
                <select name="kecamatan_id" required class="select-field pl-11 pr-10">
                    <option value="" >-- Pilih Kecamatan --</option>
                    @foreach($kecamatans as $opt)
                        <option value="{{ $opt->id }}" >{{ $opt->nama_kecamatan }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <div class="relative group mt-1">
                ...
                <input type="text" name="nama_desa" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all" placeholder="Nama desa atau kelurahan">
            </div>
            @error('nama_desa')<p class="mt-1.5 text-xs" >{{ $message }}</p>@enderror
        </div>
        <div class="pt-2 flex justify-end gap-3 border-t border-slate-200">
            <a href="{{ route('disperindag.desas.index') }}" class="inline-flex justify-center items-center rounded-xl bg-white px-6 py-3 text-sm font-bold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-200 hover:bg-slate-50 hover:text-slate-900 transition-all duration-200">Batal</a>
