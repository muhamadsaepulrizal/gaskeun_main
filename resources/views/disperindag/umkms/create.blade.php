    ...
    <form action="{{ route('disperindag.umkms.store') }}" method="POST" class="space-y-5">
            ...
            <div class="relative mt-1 group">
                ...
                <select name="penduduk_id" required class="select-field pl-11 pr-10">
                    ...
                    @foreach($penduduks as $opt)
                        <option value="{{ $opt->id }}" >{{ $opt->nama_lengkap }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <div class="relative group mt-1">
                ...
                <input type="text" name="nama_usaha" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all" placeholder="Nama usaha UMKM">
            </div>
        ...
        <div>
            ...
            <div class="relative group mt-1">
                <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400 group-focus-within:text-brand transition-colors"><i class="fa-solid fa-pen"></i></span>
                <input type="text" name="bidang_usaha" required class="input-field pl-11 w-full focus:ring-2 focus:ring-brand/20 transition-all" placeholder="Contoh: Kuliner, Jasa">
            </div>
